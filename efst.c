/***********************************************************************
	...
	$Id: efst.c,v 1.42 2022/11/19 13:45:51 warme Exp $

	File:	efst.c
	Rev:	e-4
	Date:	11/18/2022

	Copyright (c) 1998, 2022 by Pawel Winter, Martin Zachariasen.
...
struct gst_proplist *	plist;
	...
	const char *fname =
    (battery_filename && battery_filename[0])
        ? battery_filename
        : "batteries.txt";

	einfo.battery_info = load_battery_info(fname, nterms);
		

	if (battery_filename && battery_filename[0]) {
		einfo.battery_info = load_battery_info(battery_filename, nterms);
	} else {
		einfo.battery_info = NULL;  /* no penalty for normal run */
	}
	...
	free_battery_info(einfo.battery_info);
...
struct edge *		edges;
	...
	double battery_penalty = compute_battery_penalty(eip->battery_info, 
                                                   eip->termindex, 
                                                   eip->termlist->n);
    length += battery_penalty;
