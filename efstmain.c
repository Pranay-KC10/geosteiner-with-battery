/***********************************************************************
	...
	$Id: efstmain.c,v 1.33 2022/11/19 13:45:51 warme Exp $
...
extern char* battery_filename;  /* defined in battery_glue.c */
...
gst_scale_info_ptr	scinfo;
	...
	if (argc > 1 && argv[1] && argv[1][0]) {
		...
		if (fd < 0) {
			fprintf(stderr, "efst: unable to open '%s': %s\n", argv[1], strerror(errno));
			return 1;
		}
		if (dup2(fd, STDIN_FILENO) < 0) {
			fprintf(stderr, "efst: dup2 failed: %s\n", strerror(errno));
			close(fd);
			return 1;
		}
		close(fd);
	...
	while (argc > 0) {
		...
		while ((c = *ap++) NE '\0') {
			...
			case 'b':
				...
				battery_filename = ap;  // Add this global variable
				ap = "";
				break;
			case 'd':
				if (*ap EQ '\0') {
